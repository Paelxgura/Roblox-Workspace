...
local persistentSpeedConnection
...
MovementTab:CreateToggle({
    Name = "Persistent Speed Hack (Anti-Crawl Reset)",
    CurrentValue = false,
    Flag = "PersistentSpeedToggle",
    Callback = function(Value)
        persistentSpeedEnabled = Value
        if persistentSpeedEnabled then
            persistentSpeedConnection = RunService.Heartbeat:Connect(function()
            ...
            if persistentSpeedConnection then
                persistentSpeedConnection:Disconnect()
                persistentSpeedConnection = nil
            end
        end
    end,
})
